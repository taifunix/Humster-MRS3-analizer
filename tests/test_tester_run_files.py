...
def test_publish_run_snapshots_replaces_runs_and_configures_tester(tmp_path: Path) -> None:
    ...
    result = publish_run_snapshots(template, bot_root, tester_config, [structure], "2026-08-01", "2026-08-18", 7, AlgorithmConfig.defaults())
...
def test_publish_run_snapshots_refuses_a_runs_directory_symlink(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="symbolic"):
        publish_run_snapshots(tmp_path / "template.json", bot_root, tester_config, [{}], "2026-08-01", "2026-08-18", 1, AlgorithmConfig.defaults())
