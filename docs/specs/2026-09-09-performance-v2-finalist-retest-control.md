...
## Дополнение 2026-09-13: исходные данные для нормализации

Статус дополнения: подготовлено к реализации; основной контур ретеста сохраняется.
Использовать существующие генерацию JSON, tester inbox и IMPORT & REPLACE.
Нового загрузчика, режима панели или отдельной БД не создавать.

- Парсер сохраняет необязательные Price/Cost исполнения. В БД они записываются
  Decimal-строками в существующий `strategy_actions.raw_action_json`.
  Payload содержит версию и `price_cost_semantics=actual_fill_not_planned_position`;
  такая же отметка есть в metadata результата. Порядок и число 13 колонок действий
  сохраняются, заменяется только значение последнего существующего поля.
  Пустые/отсутствующие значения остаются неизвестными; некорректные необязательные
  значения помечаются `invalid_fields`, не превращаются в нули и не ломают
  импорт корректных основных фактов старого отчёта.
- Единственная новая колонка — nullable
  `strategy_results.optimizer_source_metadata_json VARCHAR`. Добавляется как
  при создании, так и при повторной инициализации существующей v4 БД.
  Версия схемы и состав таблиц/индексов сохраняются. Колонки действий не добавлять:
  старый импорт записывает их позиционно.
- Снимок настроек включает только `exchange.use_upnl/use_frozen_balance` и
  `basic.use_fix/my_fix_balance/balance_percentage_long/balance_percentage_short/
  risk_long/risk_short/lot_long/lot_short/max_balance/leverage/cross_margin`.
  Boolean сохраняется только как boolean, конечные числовые значения — строками.
  Сведения об аккаунте, уведомлениях и остальные настройки не копировать.
- JSON имеет сортированные ключи и компактные разделители. Пределы — 16 KiB
  на metadata результата и 2 KiB на действие; превышение означает диагностируемое
  отсутствие данных, не усечение числа. Decimal сохраняется через `str`, включая
  точную экспоненциальную запись без разворачивания огромных показателей степени.
  Детерминизм metadata проверяется при одинаковой ревизии/времени импорта;
  новый импорт закономерно меняет provenance timestamp.
- Metadata имеет собственную версию, SHA-256 исходного отчёта из проверенного
  manifest и точное `imported_at_utc` результата. Чтение проверяет совпадение
  ревизии с текущей строкой; несовпадение, повреждение JSON или неизвестная версия
  означают недоступные metadata. При предоставленном текущем hash проверить и его.
  Это защищает от старых metadata после REPLACE прежней версией импортёра.
- Обновлённый ADD/REPLACE сохраняет дополнительные данные в той же транзакции,
  что и основные результаты; REPLACE перезаписывает или очищает их. Отсутствие
  новых полей не меняет identity и пользовательские решения. Исходные индексы
  действий сохраняются после исключения начала отчёта.
- Проверить старые и новые отчёты, миграцию/idempotence, process-pool передачу,
  ADD/REPLACE, откат, сохранение ручных решений и отклонение устаревших metadata
  на временных БД. Рабочий ретест/миграция выполняются отдельно после backup.

Это минимальное сохранение исходных фактов общим импортом, которым пользуется
