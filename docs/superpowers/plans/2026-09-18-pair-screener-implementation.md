...
**Статус (обновлено 2026-09-19):** ✅ Этапы 0–6 закрыты (вся спека
реализации). `errors.py`,
`registry.py` (включая `list_unscreened_symbols` для кнопки "загрузить
неотскринированные пары", добавлено на старте Этапа 5), `listing.py`
(реестр → dates.xlsx → Bybit по-символьно с троттлингом ≥250мс и
распознаванием HTTP 403), `evaluate.py` (вердикты по CSV, обе стороны),
`panel_testing.py`/`panel.py` (контроллер-методы SCREENER 01, роуты
`/api/v2/testing/screener/*`), `index.html`/`app.js` (карта `#screener-local`,
полная обвязка, живая сквозная HTTP-проверка) — всё реализовано.
`tests/screener` + `tests/test_panel_testing.py` + `tests/runner` +
`tests/test_panel.py` + `tests/test_panel_static_ui.py` — 531/531 зелёных
(1 skip не связан со скринером). Каждый файл прошёл несколько раундов
независимого review до схождения. Этап 4 закоммичен локально (`2c0b644`, только `main`, без push); Этапы 5-6
и retroactive-фикс к Этапу 4 (`list_unscreened_symbols`, `requestJson`
allow-list) пока в pending changes — ожидают следующего запроса на коммит.
Все шесть этапов плана (0-6) закрыты; реализация SCREENER 01 завершена и
подтверждена реальным сквозным прогоном на продовых данных (см. Этап 6).
...
папка отчётов тестера `tester/report/my_test` (16 пар, 4864 строки
...
**Задним числом добавлено при старте Этапа 5** (раздел 6.6 спеки описывает
кнопку загрузки ещё не отскринированных пар из реестра, которая была
пропущена в Этапе 4): `src/mrs3/screener/registry.py::list_unscreened_symbols(path,
side)` — читает лист `Пары` и лист `Скрининг` ОДНИМ открытием книги через
openpyxl (`read_only=True`), без pandas (упрощение после review — исходная
версия открывала файл дважды, pandas+openpyxl); отклоняет дубли символов в
`Пары` тем же способом, что `read_registry_listing_dates` (иначе кнопка могла
бы предложить пару, на которой позже гарантированно упадёт `evaluate`);
проверяет заголовок листа `Скрининг` перед позиционным чтением колонок (фикс
review — без этого рассинхронизация заголовка молча читалась бы как валидные
данные); безопасно обрабатывает пустой лист без заголовка (`StopIteration`→
понятная ошибка, тоже фикс review). `panel.py::local_screener_registry_pairs`
+ роут `GET /api/v2/testing/screener/registry-pairs`. Тесты — 20 в
`tests/screener/test_registry.py`, 3 в `tests/test_panel_testing.py`.
Прошло 4 раунда независимого review (включая параллельный 5-агентный запуск)
до схождения.

**Этап 5 (feat, UI) — ✅ закрыт.** `index.html`: карта `#screener-local`
(рядом с `#runner-local` в том же `.runner-grid`) — textarea пар, селектор
LONG/SHORT, даты, чекбокс очистки отчётов (по умолчанию включён — раздел 5
спеки), кнопка загрузки неотскринированных пар из реестра, кнопки
Проверить/Подготовить/Старт/Стоп/Оценить, блок ожидаемого числа прогонов
(`#screener-expected-runs`, заполняется из ответа fill — раздел 5 спеки
говорит "до старта", не "живое превью по мере ввода", поэтому отдельный
live-превью endpoint не понадобился), чекбокс "Включая CHECK при передаче",
кнопка "Передать в RUNNER 01", таблица вердиктов (`.table-wrap > table`, по
образцу shortlist, без вложенных групп — одна строка на пару/сторону) и
ссылка-экспорт. `app.js`: обработчики fill/check/start/stop/evaluate/export/
registry-pairs-load/transfer; `renderScreenerVerdicts()` рендерит плоскую
таблицу; экспорт — blob-download по образцу `finalist-retest-export`;
"Передать в RUNNER 01" — чисто клиентская сортировка по числу хороших точек,
без нового endpoint'а (сторона берётся из вердиктов, не из селектора — все
вердикты одного успешного `evaluate` гарантированно одной стороны, это
инвариант раздела 7 спеки, проверено чтением `_detect_side` в `evaluate.py`).
Отдельный фикс вне карты: `requestJson()` в `app.js` глушил все тексты
ошибок `/api/v2/testing/screener/*` до generic "Server validation failed."
(allow-list проверял только endpoint'ы `/fresh/`/`/surfaces/` и коды из
заглавных букв) — из-за этого уже реализованные в Этапе 4 осмысленные
сообщения (USDT, смешение дат/сторон, Bybit 403, "реестр не настроен" и
т.д.) никогда бы не дошли до оператора; добавлен `/api/v2/testing/screener/`
в allow-list, это касается ВСЕХ screener-эндпоинтов, не только новой кнопки.
`app.css` не потребовал изменений — карта переиспользует существующие
классы (`.panel-card.runner-card`, `.table-wrap`, `.shortlist-table`,
`.button-row`, `.field-group`, `.check`).

Проверка: `node --check app.js` — OK; `tests/test_panel_static_ui.py` — 97
passed (один найденный по пути конфликт — переменная `token` в JS ловилась
project-wide запретом на слово "token" в `app.js`, переименована в `entry`);
полный прогон `tests/screener tests/test_panel_testing.py tests/runner
tests/test_panel.py tests/test_panel_static_ui.py` — 531 passed, 1 skipped
(skip не связан со скринером). Реальная сквозная HTTP-проверка (не только
unit-тесты): поднят настоящий `create_panel_server` на localhost с изолированным
tmp-конфигом (`panel.default_root=static`, без реального `bot_root`/реестра),
настоящими сокет-запросами проверены `GET /` (карта присутствует в HTML),
`GET /panel-web/app.js` (обвязка присутствует), `GET .../status`,
`GET .../registry-pairs` (понятная 400-ошибка без настроенного реестра),
`POST .../fill` (реальный рендер, `expected_runs=608` для 2 пар LONG),
`POST .../evaluate` (вердикт GO с ожидаемыми полями) и `GET .../export`
(скачиваемый CSV с верными данными) — весь путь end-to-end отработал
корректно. Реальный тестер (кнопка "Старт" против реального бота из
`tester_runner.bot_root`)
не запускался — это требует отдельной явной просьбы пользователя.
Обнаружено при review, но НЕ является багом (проверено): backend уже
гарантирует единственную сторону на успешный `evaluate` (раздел 7 спеки),
поэтому кнопка "Передать в RUNNER 01" не может ошибочно смешать
LONG/SHORT символы под одним селектором стороны.

**Этап 6 (docs) — ✅ закрыт.** Последний пункт ручной проверки плана —
повторная сверка вердиктов 16 пар после Этапа 5 — натолкнулся на реальное
осложнение: папка отчётов `tester/report/my_test` к этому моменту
смешивала исходный 4864-прогонный скрининг с отдельным ~27360-прогонным
батчем, который пользователь запустил самостоятельно уже после Этапа 3 (на
момент проверки — активно пишущим файлы). Read-only `evaluate_pairs` против
этой папки корректно отказал с "mixes more than one (StartDate, EndDate)
window" — инвариант раздела 7 сработал на реальном, незапланированном
смешении входа, как и задуман. Не стал сам ничего перемещать/удалять в
реальной папке — сообщил пользователю; он вручную перенёс исходные 4864
HTML-отчёта в соседнюю `my_test1` (файлы активного батча не тронуты).
Read-only `evaluate_pairs` против `my_test1` (реальный `config.local.json`,
реальные `AlgorithmConfig`/`ScreenerConfig`, реальный `input/dates.xlsx`)
воспроизвёл таблицу от 2026-09-18 побитово: GO — MSTRUSDT, KORUUSDT,
SOXLUSDT; CHECK (8) — INTCUSDT, SKHYUSDT, SNDKUSDT, CRCLUSDT, CLUSDT,
SNXXUSDT, TSLAUSDT, XAGUSDT; STOP (5) — AAPLUSDT, NVDAUSDT, XAUUSDT,
BZUSDT, GOOGLUSDT; INCOMPLETE — 0. Это полный продовый код-путь (Этапы
3-5 вместе), не синтетическая фикстура. `PRD.md` обновлён: статус строки
реестра — "Implemented / verified on production sample", с явной ссылкой на
эту сверку и на всё ещё открытые, осознанно принятые риски (раздел 9
пропущен, пороги не подтверждены за пределами исходных 16 пар). Статус
DRAFT в самой спеке намеренно не менялся — он про пороги, не про код, и это
условие всё ещё верно. `progress.md` обновлён тем же итогом.
...
- Ручная сверка вердиктов на реальных
  `tester/report/my_test/reports_history.csv` после этапа 3 и снова после
  этапа 5 (GO:
