...
Keep one persisted `workers` setting for both HTML Preflight snapshot/hash
workers and import parsing workers. A separate `preflight_workers` setting is
explicitly rejected as unnecessary configuration complexity; the single
DuckDB writer remains unchanged.
