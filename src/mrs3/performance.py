...
def parse_performance_report(source: bytes) -> ParsedPerformanceReport:
    ...
    action_times = [_timestamp(row["Timestamp"]) for row in actions]
    series_times = [datetime.fromtimestamp(point[0] / 1000, tz=timezone.utc) for point in wallet]
    all_times = action_times + series_times
    inventory = PerformanceInventory(
    ...
    if inventory.trade_row_count != len(actions) or inventory.wallet_sample_count != len(wallet):
