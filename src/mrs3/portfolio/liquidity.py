...
class ReferenceReader:
    ...
    def from_records(cls, *, instruments: Iterable[Mapping[str, Any]], risk_tiers: Iterable[Mapping[str, Any]], captured_at_ms: int) -> ReferenceSnapshot:
        ...
        for item in instruments:
            ...
            parsed_instruments.append(Instrument(symbol, str(get(item, "status", required=False) or ""), str(get(item, "contract_type", "contractType")), _decimal(get(item, "tick_size", "tickSize", "priceFilter.tickSize"), "tick_size", positive=True), _decimal(get(item, "qty_step", "qtyStep", "lotSizeFilter.qtyStep"), "qty_step", positive=True), _decimal(get(item, "min_qty", "min_order_qty", "minOrderQty", "lotSizeFilter.minOrderQty"), "min_qty", positive=True), _decimal(get(item, "max_qty", "max_limit_order_qty", "maxLimitOrderQty", "max_order_qty", "maxOrderQty", "lotSizeFilter.maxLimitOrderQty", "lotSizeFilter.maxOrderQty"), "max_qty", positive=True), _decimal(get(item, "leverage_step", "leverageStep", "leverageFilter.leverageStep"), "leverage_step", positive=True), _decimal(get(item, "max_leverage", "maxLeverage", "leverageFilter.maxLeverage"), "max_leverage", positive=True), _decimal(get(item, "min_notional", "min_notional_value", "minNotionalValue", "lotSizeFilter.minNotionalValue"), "min_notional", positive=True) if get(item, "min_notional", "min_notional_value", "minNotionalValue", "lotSizeFilter.minNotionalValue", required=False) is not None else None))
        def optional_decimal(item: Mapping[str, Any], *names: str) -> Decimal | None:
            value = get(item, *names, required=False)
            return None if value is None or (isinstance(value, str) and not value.strip()) else _decimal(value, names[0], nonnegative=True)

        parsed_tiers: list[RiskTier] = []
        for item in risk_tiers:
            parsed_tiers.append(RiskTier(str(get(item, "symbol")), _decimal(get(item, "risk_limit_value", "riskLimitValue"), "risk_limit_value", positive=True), _decimal(get(item, "max_leverage", "maxLeverage"), "max_leverage", positive=True), int(get(item, "risk_id", "riskId", "id", required=False)) if get(item, "risk_id", "riskId", "id", required=False) is not None else None, optional_decimal(item, "maintenance_margin", "maintenanceMargin"), optional_decimal(item, "initial_margin", "initialMargin"), optional_decimal(item, "mm_deduction", "mmDeduction")))
