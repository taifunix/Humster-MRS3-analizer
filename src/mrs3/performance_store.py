...
CREATE TABLE IF NOT EXISTS dd5_results (dd5_run_id VARCHAR NOT NULL, test_run_id VARCHAR NOT NULL, projected_pnl_dd5 DECIMAL(38,12), projected_dd_pct DECIMAL(38,12), projected_pnl30_dd5 DECIMAL(38,12), scaled_lots_json VARCHAR, capital_requirement_proxy DECIMAL(38,12), holding_filter VARCHAR, pareto_rank INTEGER, PRIMARY KEY (dd5_run_id, test_run_id));
CREATE OR REPLACE VIEW latest_backtest_by_strategy_version AS SELECT * FROM backtest_runs;
CREATE OR REPLACE VIEW dd5_latest_results AS SELECT * FROM dd5_results;
CREATE OR REPLACE VIEW portfolio_layer_a_input AS SELECT r.test_run_id, r.strategy_version_id, r.period_start_utc, r.period_end_utc, m.total_pnl_pct, m.max_drawdown_pct, FALSE AS portfolio_event_ready FROM backtest_runs r LEFT JOIN backtest_metrics m USING (test_run_id);
"""
