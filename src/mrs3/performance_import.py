...
def _write_import_rows(connection: duckdb.DuckDBPyConnection, import_id: str, prepared: list[dict[str, object]], manifest: dict[str, object]) -> tuple[int, int]:
    ...
    for item in prepared:
        ...
        connection.execute(
            ...
            [item["strategy_id"], parsed.settings["name"], basic.get("symbol", ""), basic.get("side", strategy.get("side", "")), basic.get("time_frame", ""), item["settings_json"], datetime.now(timezone.utc)],
        )
        connection.execute(
...
def import_performance_batch(request: PerformanceImportRequest) -> PerformanceImportResult:
    ...
    try:
        ...
        with duckdb.connect(str(request.database), read_only=True) as connection:
            for item in prepared:
                if connection.execute("select count(*) from backtest_runs where test_run_id = ? and result_payload_sha256 = ?", [item["test_run_id"], item["payload_hash"]]).fetchone()[0] != 1:
    ...
    _write_checklist(inbox, [{"manifest_entry_id": str(item["entry"]["manifest_entry_id"]), "report_path": str(item["entry"]["report_path"]), "source_html_sha256": str(item["source_hash"]), "status": "IMPORTED", "safe_to_delete": "YES", "cleanup_state": "DELETE_READY", "deleted_at_utc": ""} for item in prepared])
