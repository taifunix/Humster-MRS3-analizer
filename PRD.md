...
Базовый DuckDB surface coverage review реализован, но [каноническая спецификация](docs/specs/2026-08-14-duckdb-surface-coverage-review.md) повторно открыта утверждённым [ADR-0008](docs/decisions/0008-common-close-ma-readiness-and-degenerate-row-isolation.md). Ожидающая реализация должна выбирать общий readiness-интервал для всех Close MA `2..7`, хранить по одному witness на Close MA и игнорировать только структурно нулевые report/grid-строки. До этой реализации панель использует прежний one-MA-pair/global-empty контракт ADR-0007. Preflight progress/elapsed UI и `coverage_summary.csv` остаются отложенными.
...
| Approved contract change / implementation pending | [DuckDB surface coverage review](docs/specs/2026-08-14-duckdb-surface-coverage-review.md) | common Close MA `2..7` readiness interval, six ordered witnesses and narrow degenerate-row isolation; existing sparse V2 implementation remains on the earlier contract until updated | DuckDB analysis storage, [ADR-0007](docs/decisions/0007-observed-sparse-surface-contract.md), [ADR-0008](docs/decisions/0008-common-close-ma-readiness-and-degenerate-row-isolation.md) |
