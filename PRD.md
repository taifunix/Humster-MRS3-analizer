...
Базовый DuckDB surface coverage review реализован. Приоритетом является точечный operational patch [канонической спецификации](docs/specs/2026-08-14-duckdb-surface-coverage-review.md): double-zero isolation, stale-token clearing, диагностируемые direct jobs, ordinal и coverage-artifact links при сохранении текущего one-MA-pair контракта. Реализация [ADR-0008](docs/decisions/0008-common-close-ma-readiness-and-degenerate-row-isolation.md) и её [проект плана](docs/superpowers/plans/2026-08-15-common-close-ma-readiness.md) заморожены до отдельной ревизии, которая должна включить MA-C, `coverage_summary.csv`, progress feedback, оптимизацию повторных source passes и единый preview/real-preflight lifecycle.
...
| Priority-1 operational patch / implementation pending | [DuckDB surface coverage review](docs/specs/2026-08-14-duckdb-surface-coverage-review.md) | unblock and diagnose the current one-MA-pair flow; preview and real preflight must reproduce the same unchanged-source scopes and intervals; ADR-0008 is frozen | DuckDB analysis storage, [ADR-0007](docs/decisions/0007-observed-sparse-surface-contract.md), [ADR-0008](docs/decisions/0008-common-close-ma-readiness-and-degenerate-row-isolation.md) |
