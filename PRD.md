...
| DRAFT / implementation started | [Pair screener](docs/specs/2026-09-18-pair-screener.md), [implementation plan](docs/superpowers/plans/2026-09-18-pair-screener-implementation.md) | дешёвый 304-точечный прогон на пару для отсева пар до полного 5472-точечного сбора; CSV-only оценка (6.1a закрыт), новый экран SCREENER 01 | RUNNER 01/`LocalTestingService`; полная кросс-валидация раздела 9 пропущена по решению пользователя, пороги не подтверждены |
